-- Sistema de Progresso Avançado - Tabelas Adicionais
...
CREATE TABLE badges_dinamicas (
    id INT AUTO_INCREMENT PRIMARY KEY,
    nome VARCHAR(100),
    descricao TEXT,
    icone VARCHAR(50),
    categoria ENUM('performance', 'consistencia', 'conquista', 'especial'),
    criterios JSON, -- Critérios dinâmicos em JSON
    pontos_recompensa INT DEFAULT 0,
...
CREATE TABLE insights_usuario (
    id INT AUTO_INCREMENT PRIMARY KEY,
    usuario_id INT,
    tipo_insight ENUM('performance', 'consistencia', 'disciplina', 'meta', 'recomendacao'),
    titulo VARCHAR(200),
    mensagem TEXT,
    dados_suporte JSON, -- Dados que geraram o insight
...
CREATE TABLE comparacoes_usuario (
    id INT AUTO_INCREMENT PRIMARY KEY,
    usuario_id INT,
    tipo_comparacao ENUM('ranking_geral', 'ranking_disciplina', 'ranking_nivel', 'ranking_tempo'),
    posicao_atual INT,
    total_participantes INT,
    percentil DECIMAL(5,2), -- Percentil do usuário
    dados_comparacao JSON, -- Dados detalhados da comparação
    data_calculo TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (usuario_id) REFERENCES usuarios(id)
);

-- Tabela de histórico de progresso (para gráficos e análises)
...
INSERT INTO badges_dinamicas (nome, descricao, icone, categoria, criterios, pontos_recompensa, raridade) VALUES
...
CREATE INDEX idx_progresso_disciplina_usuario ON progresso_disciplina(usuario_id);
CREATE INDEX idx_sessoes_estudo_usuario ON sessoes_estudo(usuario_id);
CREATE INDEX idx_historico_progresso_usuario ON historico_progresso(usuario_id);
CREATE INDEX idx_insights_usuario ON insights_usuario(usuario_id, visualizado);
CREATE INDEX idx_metas_usuario ON metas_usuario(usuario_id, ativa);
CREATE INDEX idx_usuarios_badges_usuario ON usuarios_badges(usuario_id);
